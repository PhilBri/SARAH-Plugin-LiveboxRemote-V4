...
function sendData (cmdTab, cfgLiveBox, clbk) {
    ...
    require('request')(options, function (err, response, body) {
        if ( !err && response.statusCode == 200 ) {
            ...
            if (cmdTab.length) {
                var lbTimer = lbData.stby == 1 || !cmd.indexOf('operation=09') ? 2500 : 200;
                if (cmdTab.length == 1)
                    setTimeout( function() { sendData (cmdTab, cfgLiveBox, clbk) }, lbTimer);
                else
                    setTimeout( function() { sendData (cmdTab, cfgLiveBox, clbk) }, 200);
            }
        }
